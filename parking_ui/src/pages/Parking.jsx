            ...
            <div key={entry._id}>
              <div className="relative bg-white rounded-lg shadow-lg p-6 mb-4 h-72 hover:bg-purple-200">
                ...
                <p className="opacity-60">
                  {entry.startTime
                    ? moment(entry.startTime).format("Y-M-D, h:mm a")
                    : "N/A"}
                </p>
                {entry.status !== "Parked" && (
                  <>
                    <p className="opacity-60">
                      {entry.endTime
                        ? moment(entry.endTime).format("Y-M-D, h:mm a")
                        : "N/A"}
                    </p>
                    <p>
                      Total Amount: Rs.{" "}
                    ...
                    {entry?.status !== "Confirmed" ? (
                      ...
                      <p className="text-green-700 mt-3 w-full font-semibold">
                        Paid
                      </p>
                    )}
                  </>
                ...
                {entry.status === "Parked" ? (
                  ...
                  entry?.payment?.remainingAmount ? (
                    <UpdatePaymentModal
                      id={entry._id}
                      totalAmount={entry.payment.remainingAmount}
                    />
                  ) : (
                    <PaymentModal
                      id={entry._id}
                  ...
                  <div className="w-full absolute bottom-0 left-0 px-4  my-4 cursor-pointer rounded-full">
                    <p
                      className="bg-red-600 hover:bg-red-700 text-white text-center px-4 py-2 rounded-md "
                      onClick={() => deleteParkings(entry._id)}
                    >
                      Delete Parking
                    </p>
